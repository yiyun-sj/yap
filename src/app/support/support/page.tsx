...
export default function Support() {
  ...
  const [isSubmitLoading, submitSupport] = useLoadable(async () => {
    const { error } = await supabase
      .from('supports')
      .insert({
        type: 'support',
        message: `Support request from ${user?.email}: ${supportMsg}`,
      })
    if (error) {
  ...
  return (
    <main className='flex flex-1 flex-col items-center gap-4 p-8'>
      ...
      <text className='text-xs font-body font-light max-w-96'>
        ...
        to login here and create a support request. Alternatively, you can email
        information.yap@gmail.com for support. Thanks!
      </text>
      <form
        ...
        {user && (
          <div>
            ...
            <textarea
              ...
              className='text-sm p-2 focus:outline-none w-full text-dark'
              rows={5}
              placeholder='Please provide some details on what you need support with. We will reach out to you via the email above.'
              value={supportMsg}
